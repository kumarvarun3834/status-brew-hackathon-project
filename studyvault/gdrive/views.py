...
from django.shortcuts import render
from .menuurl import generate_menu_html

def menu_view(request):
    menu_data = read_menu_data("path/to/your/menu.txt")
    menu_html = generate_menu_html(menu_data)
    return render(request, "menu_template.html", {"menu_html": menu_html})

def folder_view(request, semester=None, subject=None, year=None):
    context = {
        'semester': semester,
        'subject': subject,
        'year': year,
        'subfolders': [],  # Fetch subfolders based on the current level
        'files': [],  # Fetch files based on the current level
    }
    # Logic to populate subfolders and files based on the current context
